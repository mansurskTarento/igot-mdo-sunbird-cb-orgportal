...
import { SelectedDesignationPopupComponent } from '../../designation/dialog-boxes/selected-designation-popup/selected-designation-popup.component'
...
export class ImportDesignationComponent implements OnInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['loader'].currentValue === false) {
      this.frameworkInfo = this.directoryService.frameWorkInfo
  ...
  loadDesignations(searchKey: string = '') {
    ...
    this.apiSubscription = this.directoryService.getIgotMasterDesignations(requestParams).subscribe(
      (result: any) => {
  ...
  selectDesignation(index: number) {
    ...
    if (designation && !designation.isOrgDesignation) {
      ...
      if (checked) {
        ...
        this.directoryService.updateSelectedDesignationList(this.selectedDesignationsList)
      } else {
    ...
    if (this.selctedDesignationsCount) {
      ...
      const observables = this.selectedDesignationsList.map((selectedDesignation: any) => {
        const requestBody = {
          ...
          code: this.directoryService.getUuid,
        ...
        return this.directoryService.createTerm(requestBody).pipe(
          map((response: any) => {
  ...
  updateTerms(orgCategorie: any, retry = false) {
    if (this.selectedDesignationsList.length === this.designationsImportFailed.length) {
      ...
      this.directoryService.updateTerms(framework, category, categoryTermCode, requestBody).subscribe({
        next: response => {
          if (response) {
  ...
  ngOnDestroy(): void {
    this.directoryService.updateSelectedDesignationList([])
    this.loaderService.changeLoaderState(false)
    if (this.apiSubscription) {
      this.apiSubscription.unsubscribe()
    }
